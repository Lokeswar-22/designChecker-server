...
export class RuleEngineService {
  ...
  private async queryGraphQL(
    ...
    try {
      ...
      const response = (await firstValueFrom(
        this.http.post(this.endpoint1, requestBody, {
          headers: {
  ...
  async getDoorsInstance(elementGroupId: string, accUserId: string) {
    ...
    const filter =
      "property.name.category==Doors and 'property.name.Element Context'==Instance";
    let cursor: string | null = null;
    const doorsWithWidth: Array<{
      id: string;
      name: string;
      elementID: any;
      FamilyName: string;
    ...
    do {
      ...
      for (const el of block?.results ?? []) {
        const familyName = el.properties?.results?.find(
          (p: any) => p.name === 'Family Name',
        );
        const elementID = el.properties?.results?.find(
          (p: any) => p.name === 'Revit Element ID',
        );
        if (elementID) {
          doorsWithWidth.push({
            id: el.id,
  ...
  async getDoorsType(elementGroupId: string, accUserId: string) {
    ...
    const filter =
      "property.name.category==Doors and 'property.name.Element Context'==Type";
    let cursor: string | null = null;
    const doorsWithWidth: Array<{
      id: string;
      name: string;
    ...
    do {
      ...
      const block = resp?.elementsByElementGroup;
      for (const el of block?.results ?? []) {
        const widthProp = el.properties?.results?.find(
          (p: any) => p.name === 'Width',
        );
        const familyName = el.properties?.results?.find(
          (p: any) => p.name === 'Family Name',
  ...
  async getCachedDoorData(
    elementGroupId: string,
    accUserId: string,
  ): Promise<CachedDoorData> {
  ...
  async getFailedDoorValidations(
    elementGroupId: string,
    accUserId: string,
  ...
  async getFailedDoorValidationsWithElements(
    elementGroupId: string,
    accUserId: string,
  ): Promise<ValidationResult[]> {
    const response = await this.executeRule(elementGroupId, accUserId);
    return response.validationResults.filter(
      (result) => !result.isValid && result.elementIds.length > 0,
    );
  }
  ...
  async getFailedElementIds(
    ...
    const failedResults = await this.getFailedDoorValidationsWithElements(
      elementGroupId,
      accUserId,
    );
  ...
  async getRampsInstance(elementGroupId: string, accUserId: string) {
    ...
    const filter =
      "property.name.category==Ramps and 'property.name.Element Context'==Instance";
    let cursor: string | null = null;
    const doorsWithWidth: Array<{
      id: string;
      name: string;
  ...
  async getRampsType(elementGroupId: string, accUserId: string) {
    ...
    const filter =
      "property.name.category==Ramps and 'property.name.Element Context'==Type";
    let cursor: string | null = null;
    const doorsWithWidth: Array<{
      id: string;
      name: string;
    ...
    do {
      ...
      for (const el of block?.results ?? []) {
        const widthProp = el.properties?.results?.find(
          (p: any) => p.name === 'Ramp Max Slope (1/x)',
        );
        const xyzProp = el.properties?.results?.find(
          (p: any) => p.name === 'Maximum Incline Length',
        );
        const familyName = el.properties?.results?.find(
          (p: any) => p.name === 'Family Name',
        );
  ...
  async getParkingInstance(
    ...
    do {
      ...
      const block = resp?.elementsByElementGroup;
      for (const el of block?.results ?? []) {
        const LevelName = el.properties?.results?.find(
          (p: any) => p.name === 'Host',
        );
        const elementID = el.properties?.results?.find(
          (p: any) => p.name === 'Revit Element ID',
  ...
  async getCachedParkingData(
    elementGroupId: string,
    accUserId: string,
    Level: string,
  ): Promise<CachedParkingData> {
    const cacheKey = `Cache Key:GET:/rule-engine/getParkingData/${elementGroupId}?accUserId=${accUserId}`;
    const cachedData = (await this.redis.get(
      cacheKey,
    )) as CachedParkingData | null;

    if (cachedData) {
      try {
        const parsedData = JSON.parse(
          cachedData as unknown as string,
        ) as CachedParkingData;
        return parsedData;
      } catch (error) {
        await this.redis.del(cacheKey);
      }
  ...
  async executeRule3(
    ...
    try {
      const parkingJson = await this.getCachedParkingData(
        elementGroupId,
        accUserId,
        levelName,
      );

      const handicappedSpaces = parkingJson.res.filter(
        (parking) =>
          parking.name.toLowerCase().includes('hcp') ||
          parking.name.toLowerCase().includes('handicapped'),
      );

      const normalSpaces = parkingJson.res.filter(
        (parking) =>
          !parking.name.toLowerCase().includes('hcp') &&
          !parking.name.toLowerCase().includes('handicapped'),
      );
  ...
  async getRampInstance(elementGroupId: string, accUserId: string) {
    ...
    do {
      ...
      const block = resp?.elementsByElementGroup;
      for (const el of block?.results ?? []) {
        const width = el.properties?.results?.find(
          (p: any) => p.name === 'Width',
        );
        const elementID = el.properties?.results?.find(
          (p: any) => p.name === 'Revit Element ID',
  ...
  async getStairsType(elementGroupId: string, accUserId: string) {
    ...
    const filter =
      "property.name.category==Stairs and 'property.name.Element Context'==Type";
    let cursor: string | null = null;
    const stairsWithWidth: Array<{
      id: string;
      name: string;
      FamilyName: string;
      stairsMaxRiserHeight: any;
    ...
    do {
      ...
      const block = resp?.elementsByElementGroup;
      for (const el of block?.results ?? []) {
        const stairsMaxRiserHeight = el.properties?.results?.find(
          (p: any) => p.name === 'Maximum Riser Height',
        );
        const familyName = el.properties?.results?.find(
          (p: any) => p.name === 'Family Name',
        );
  ...
  async executeRule5(
    ...
    try {
      const stairsData = await this.getCachedStairsData(
        elementGroupId,
        accUserId,
      );
      ...
      const validationResults: ValidationResult[] = typeData.map(
        (stairsType) => {
