...
export class RuleDataService {
    ...
    async checkRuleData(accUserId: string, elementGroupId: string) {
        try {
            ...
            if(validateAccUser) {
                const data1 = await this.rule1Repository.find({where: {accUserId: accUserId, elementGroupId: elementGroupId}});
                const data2 = await this.rule2Repository.find({where: {accUserId: accUserId, elementGroupId: elementGroupId}});
                const data3 = await this.rule3Repository.find({where: {accUserId: accUserId, elementGroupId: elementGroupId}});
                const data4 = await this.rule4Repository.find({where: {accUserId: accUserId, elementGroupId: elementGroupId}});
                const data5 = await this.rule5Repository.find({where: {accUserId: accUserId, elementGroupId: elementGroupId}});
